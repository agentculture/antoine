"""Tests for AntoineError and the exit-code policy."""
...
def test_seer_error_is_an_exception() -> None:
    err = AntoineError(code=1, message="bad input", remediation="try --help")
    assert isinstance(err, Exception)
    assert str(err) == "bad input"


def test_seer_error_to_dict() -> None:
    err = AntoineError(code=2, message="missing tool", remediation="install it")
    assert err.to_dict() == {
        "code": 2,
...
def test_remediation_defaults_to_empty() -> None:
    err = AntoineError(code=1, message="x")
