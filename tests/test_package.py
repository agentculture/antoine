...
import antoine
from antoine import _resolve_version


def test_version_is_a_nonempty_string() -> None:
    assert isinstance(antoine.__version__, str)
    assert antoine.__version__
...
def test_resolve_version_handles_each_dual_publish_dist_name(monkeypatch) -> None:
    ...
    for dist_name in ("antoine", "kata-cli", "code-lens-cli"):
        monkeypatch.setattr(
            "antoine.packages_distributions",
            lambda dn=dist_name: {"antoine": [dn]},
        )
        monkeypatch.setattr(
            "antoine._v",
...
def test_resolve_version_falls_back_through_known_dist_names(monkeypatch) -> None:
    ...
    monkeypatch.setattr("antoine.packages_distributions", lambda: {})
    ...
    monkeypatch.setattr("antoine._v", fake_v)
...
def test_resolve_version_local_fallback_when_nothing_resolves(monkeypatch) -> None:
    ...
    monkeypatch.setattr("antoine._v", always_missing)
